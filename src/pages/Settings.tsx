...
const platformConfig: Record<
  Platform,
  { name: string; description: string; icon: JSX.Element; connectLabel: string }
> = {
  gmail: {
    name: 'Gmail',
  ...
  slack: {
    name: 'Slack',
    description: 'Connect your Slack workspace to monitor channels and DMs for action items.',
  ...
  granola: {
    name: 'Granola',
    description: 'Import meeting notes from Granola to extract commitments and action items.',
    connectLabel: 'Connect Granola',
    icon: (
      <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none">
...
function ConnectSlackModal({ onConnect, onClose }: { onConnect: (workspace: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 mx-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-5">
          {platformConfig.slack.icon}
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Connect Slack</h3>
        </div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          In production, this would redirect to Slack OAuth. For now, enter your workspace name to simulate the connection.
        </p>
        <input
          type="text"
          value={workspace}
          ...
          className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent mb-4"
          autoFocus
          onKeyDown={(e) => e.key === 'Enter' && workspace.trim() && onConnect(workspace.trim())}
        />
        <div className="flex gap-3">
          <button
            onClick={() => workspace.trim() && onConnect(workspace.trim())}
...
function ConnectGranolaModal({ onConnect, onClose }: { onConnect: () => void; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 mx-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-5">
          {platformConfig.granola.icon}
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Connect Granola</h3>
        </div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Granola doesn't have a public API yet. To connect, you can forward your Granola meeting summary emails to your unique Omnibox ingest address.
        </p>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-4">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Your ingest email</p>
          <p className="text-sm font-mono text-gray-900 dark:text-white">notes-demo@omnibox.ai</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Forward your Granola emails here and we'll automatically extract meeting notes and action items.</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={onConnect}
...
export function Settings() {
  ...
  const handleConnectSlack = (workspace: string) => {
    connectPlatform('slack', { workspace });
    setConnectingPlatform(null);
  };

  const handleConnectGranola = () => {
    connectPlatform('granola', { forwardingEmail: 'notes-demo@omnibox.ai' });
    setConnectingPlatform(null);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
                  ...
                  <div className="flex items-center gap-4">
                      ...
                      {isConnected && integration.platformMetadata && (
                        <div className="flex items-center gap-3 mt-2 text-xs text-gray-400 dark:text-gray-500">
                          {integration.platformMetadata.email && (
                            <span>{integration.platformMetadata.email as string}</span>
                          )}
                          {integration.platformMetadata.workspace && (
                            <span>{integration.platformMetadata.workspace as string}</span>
                          )}
                    ...
                    <div className="flex-shrink-0">
                      {isConnected ? (
                        ...
                        <button
                          onClick={() => setConnectingPlatform(integration.platform)}
      ...
      {connectingPlatform === 'slack' && (
        <ConnectSlackModal onConnect={handleConnectSlack} onClose={() => setConnectingPlatform(null)} />
      )}
      {connectingPlatform === 'granola' && (
        <ConnectGranolaModal onConnect={handleConnectGranola} onClose={() => setConnectingPlatform(null)} />
      )}
    </div>
  );
}
